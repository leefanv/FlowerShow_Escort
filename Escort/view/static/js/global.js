...
$(document).ready(function(){
    ...
    $('#pay_submit').click(function(){
    	...
    	if(bdInformation.replace(blankReg,'') ==''){
            ...
            'bd_ocation_address':bdLocationAddress, 'bd_information':bdInformation,'bd_service_fee':bdServiceFee,
            'bd_tip':bdTip,'bd_payindex':bdPayIndex};
            data = JSON.stringify(data); //预计数据接口  POST数据             
            $.ajax({
                type:'post',
                url:'http://seize.space/send_bd',
                dataType:'json',
                data:data,
                async:true,
    ...
    $('.m-invitation-ft .zan a').click(function(){
        if($(this).hasClass('icon-zan')){                             //取消点赞
            ...
            $.ajax({
                ...
                contentType:'application/json',
                success:function(){
                    numberFrom.text(number);numberFrom.text(number);//点赞人数加一个
                },
                error:function(XMLHttpRequest, textStatus, errorThrown){
                    alert(XMLHttpRequest.status);
                    alert(XMLHttpRequest.readyState);
